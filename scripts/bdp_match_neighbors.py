...
def _build_arg_parser():
    ...
    p.add_argument(
        "tgt_file", help="Unannotated target filename (must be supported by VTK)."
    )
    ...
    p.add_argument(
        "--out_dir", help="Output suffix (must be supported by VTK) [./]."
    )
...
def main():
    ...
    for i, src_file in enumerate(args.src_files):
        ...
        out_filename = os.path.join(
            args.out_dir, os.path.splitext(basename)[0] + args.out_suffix + ".ply"
        )
    ...
    for i, src_file in enumerate(args.src_files):
        ...
        _, curr_indices, curr_distances = match_neighbors(
            src_bdp_obj,
            tgt_bdp_obj,
    ...
    distances, indices = tree.query(
        valid_vertices, k=10, distance_upper_bound=args.distance * 5
    )
    ...
    new_annotations = np.array(
        [np.argmax(np.bincount(row)) for row in reshaped_indices]
    )
    ...
    for i, src_file in enumerate(args.src_files):
        ...
        if len(final_indices):
            ...
            save_polydata(tmp_bdp_obj.get_polydata(), out_filename, ascii=args.ascii)
        else:
