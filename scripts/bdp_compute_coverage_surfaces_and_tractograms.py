...
def _build_arg_parser():
    ...
    p.add_argument("out_json", help="Path of the output json file.")
    ...
    p.add_argument("--out_dir", help="Path of the output directory for symlinks.")
...
def main():
    ...
    in_tractograms = [f for f in args.in_files if os.path.splitext(f)[1] == ".trk"]
    in_surfaces = [f for f in args.in_files if os.path.splitext(f)[1] != ".trk"]

    if len(in_surfaces) > 1 and len(in_tractograms) > 1:
        raise ValueError(
            "Either pick multiple surfaces or multiple tractograms, not both."
        )
    ...
    if args.print_top_N:
        ...
        top_both = sorted(top_both, key=lambda x: top_both[x][0], reverse=True)
        ...
        mean_0 = np.mean(
            [x[0] for i, x in enumerate(list_of_scores) if labels[i] == 0]
        )
        mean_1 = np.mean(
            [x[0] for i, x in enumerate(list_of_scores) if labels[i] == 1]
        )
        labels_to_pick = 0 if mean_0 > mean_1 else 1
        top_both = [
            k
            for i, k in enumerate(score_result.keys())
            if labels[i] == labels_to_pick
        ]
    else:
        top_both = sorted(
            score_result, key=lambda x: score_result[x][0], reverse=True
        )
    ...
    for filename in top_both:
        if args.out_dir:
            ...
            out_filename = os.path.join(args.out_dir, os.path.basename(filename))
