...
def _build_arg_parser():
    ...
    p.add_argument("in_bundle", help="Requires the filename (.trk or .tck).")
    p.add_argument("radius", type=float, help="Radius of the tubes (mm).")
    p.add_argument("out_filename", help="Output bundle filename (.ply).")
    ...
    p.add_argument(
        "--reference", default="same", help="Reference image for the output file."
    )
...
def main():
    ...
    if os.path.isfile(args.out_filename) and not args.overwrite:
        raise IOError(f"{args.out_filename} already exists, use -f to overwrite.")
