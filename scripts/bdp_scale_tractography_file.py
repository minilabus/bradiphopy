...
def _build_arg_parser():
    ...
    p.add_argument("in_file", help="Input filename (supports .trk and all VTK).")
    p.add_argument("out_file", help="Output filename (supports .trk and all VTK).")
...
def main():
    ...
    if ext in [".trk", ".tck"]:
        ...
        sft = load_tractogram(args.in_file, ref, bbox_valid_check=False)
        sft.streamlines._data = sft.streamlines._data
        polydata = lines_to_vtk_polydata(sft.streamlines)
        save_tractogram(sft, ".tmp.vtk", bbox_valid_check=False)
    ...
    if ext in [".trk", ".tck"]:
        ...
        save_tractogram(sft, args.out_file, bbox_valid_check=False)
    else:
