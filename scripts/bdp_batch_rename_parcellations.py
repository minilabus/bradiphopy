...
def _build_arg_parser():
    ...
    p.add_argument("in_dir", help="Input directory.")
    p.add_argument("in_json", help="Input JSON file.")
    p.add_argument("out_dir", help="Output directory.")
    p.add_argument(
        "--extention", default="ply", help="File extention to use [%(default)s]."
    )
    p.add_argument(
        "-f",
        dest="overwrite",
...
def main():
    ...
    if not os.path.isdir(args.in_dir):
        raise IOError("{} does not exist.".format(args.in_dir))
    if not os.path.isfile(args.in_json):
        raise IOError("{} does not exist.".format(args.in_json))
    ...
    if value_count == key_count:
        raise ValueError(
            "Both keys and values are in the input directory.\n"
    ...
    for key, value in data.items():
        ...
        if os.path.isfile(out_file) and not args.overwrite:
            raise IOError("{} already exists, use -f to overwrite.".format(out_file))
