...
def _build_arg_parser():
    ...
    p.add_argument("in_file", help="Input filename (.nii or .nii.gz).")
    p.add_argument("out_file", help="Output filename (.ply).")
    ...
    p.add_argument(
        "--to_lps", action="store_true", help="Flip for Surfice/MI-Brain LPS"
    )
    p.add_argument(
        "--save_point_cloud",
        action="store_true",
    ...
    p.add_argument(
        ...
        type=int,
        default=0.0001,
        help="Distance between point for sampling the mesh [%(default)s].",
    )
    p.add_argument(
        "--dilate",
        type=int,
...
def main():
    ...
    if args.save_point_cloud:
        vertices = sample_mesh_to_point_cloud(
            transformFilter.GetOutput(), args.sampling_distance
        )
        obj_point_cloud = BraDiPhoHelper3D.generate_bdp_obj(vertices)
        save_polydata(
            obj_point_cloud.get_polydata(), args.out_file, ascii=args.ascii
        )
